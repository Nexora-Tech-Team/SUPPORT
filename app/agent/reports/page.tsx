...
export default async function AgentReportsPage() {
  const [allTickets, agents] = await Promise.all([
    db.ticket.findMany({ select: { status: true, priority: true, department: true, createdAt: true } }),
    db.user.findMany({
      where: { role: { in: ['SUPPORT_AGENT', 'ADMIN'] } },
      select: { id: true, name: true, _count: { select: { assignedTickets: true } } },
      orderBy: { name: 'asc' },
    }),
  ]);

  const statusCounts: Record<string, number> = {};
  for (const t of allTickets) statusCounts[t.status] = (statusCounts[t.status] ?? 0) + 1;

  const priorityCounts: Record<string, number> = {};
  for (const t of allTickets) priorityCounts[t.priority] = (priorityCounts[t.priority] ?? 0) + 1;

  const deptCounts: Record<string, number> = {};
  for (const t of allTickets) deptCounts[t.department] = (deptCounts[t.department] ?? 0) + 1;
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const count = allTickets.filter((t) => new Date(t.createdAt) >= start && new Date(t.createdAt) < end).length;
  ...
  const statusColors: Record<string, string> = { OPEN: 'bg-blue-500', IN_PROGRESS: 'bg-amber-500', PENDING: 'bg-sky-500', CLOSED: 'bg-emerald-500', RESOLVED: 'bg-emerald-400' };
  const priorityColors: Record<string, string> = { URGENT: 'bg-rose-500', HIGH: 'bg-orange-500', MEDIUM: 'bg-amber-400', LOW: 'bg-slate-400' };

  return (
    <>
      <AgentTopBar title="Reports" subtitle={`${allTickets.length} total tickets across all time`} />
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Closed', value: (statusCounts['CLOSED'] ?? 0) + (statusCounts['RESOLVED'] ?? 0), color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' },
          ].map((s) => (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {Object.entries(statusCounts).map(([status, count]) => (
                <BarChartRow key={status} label={status.replace('_', ' ')} value={count} max={maxStatus} color={statusColors[status] ?? 'bg-slate-400'} />
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {['URGENT', 'HIGH', 'MEDIUM', 'LOW'].map((p) => (
                <BarChartRow key={p} label={p} value={priorityCounts[p] ?? 0} max={maxPriority} color={priorityColors[p] ?? 'bg-slate-400'} />
              ))}
            </div>
          </div>
                ...
                return (
                  <div key={m.label} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <div className="w-full bg-slate-100 rounded-t-md overflow-hidden flex items-end" style={{ height: '100px' }}>
                      <div className="w-full bg-blue-500 rounded-t-md transition-all" style={{ height: `${Math.max(heightPct, m.count > 0 ? 4 : 0)}%` }} />
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {topDepts.map(([dept, count]) => (
                <BarChartRow key={dept} label={dept} value={count} max={maxDept} color="bg-violet-500" />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <div className="space-y-3">
            {agents.map((agent) => (
              <BarChartRow key={agent.id} label={agent.name} value={agent._count.assignedTickets} max={maxAgent} color="bg-blue-600" />
            ))}
            {agents.length === 0 && <p className="text-slate-400 text-sm">No agents found.</p>}
          </div>
        </div>
      </div>
    </>
